...
// Draws a circular progress indicator using SVG
...
// ── Color palette for dynamically added courses ──
const COURSE_COLORS = ["#3b82f6","#f59e0b","#10b981","#06b6d4","#8b5cf6","#e53e3e","#f97316","#14b8a6"];

// ── Default built-in courses shown on first load ──
...
// ── Quiz questions for each default course ──
const QUIZ_BANK = {
  py: [
...
export default function LearnifyPage() {
  // ── State ──
  const [progress, setProgress]         = useState({});       // Lesson/quiz progress per course
  const [notes, setNotes]               = useState({});       // Saved notes per course
  const [courses, setCourses]           = useState(DEFAULT_COURSES);
  const [activeCourse, setActiveCourse] = useState(null);     // Currently selected course ID
  const [tab, setTab]                   = useState("course"); // Active tab in course panel
  const [quizState, setQuizState]       = useState(null);     // Current quiz session
  const [aiSummary, setAiSummary]       = useState("");       // AI-generated summary text
  const [aiLoading, setAiLoading]       = useState(false);    // AI loading spinner
  const [noteText, setNoteText]         = useState("");       // Text in notes textarea
  const [videos, setVideos]             = useState([]);       // User-uploaded videos from backend
  const [showModal, setShowModal]       = useState(false);    // Add course modal visibility
  const [search, setSearch]             = useState("");       // Search query inside modal

  // ── Fetch user-uploaded videos from backend on mount ──
  useEffect(() => {
    api.get("/videos")
  ...
  // ── Persist progress, notes and courses to localStorage whenever they change ──
  ...
  // ── Load saved note text when switching courses or tabs ──
  useEffect(() => {
    if (activeCourse) setNoteText(notes[activeCourse] || "");
  }, [activeCourse, tab]);

  // ── Derived values for the active course ──
  const course = courses.find(c => c.id === activeCourse);
  const prog   = activeCourse
    ? (progress[activeCourse] || { done: 0, quizzesTaken: 0, avgScore: 0, watchTime: 0 })
    : null;
  const pct = course ? Math.round((prog.done / course.lessons) * 100) : 0;

  // ── Add a user video as a custom course ──
  function addVideoAsCourse(video) {
    // Prevent duplicates
    if (courses.find(c => c.id === video._id)) return;
  ...
  // ── Mark the current lesson as complete and add watch time ──
  function completeLesson() {
    setProgress(p => {
      const cur = p[activeCourse] || { done: 0, quizzesTaken: 0, avgScore: 0, watchTime: 0 };
      if (cur.done >= course.lessons) return p; // Already finished
  ...
  // ── Handle an answer selection during a quiz ──
  function answerQuiz(optIdx) {
    ...
    if (quizState.qi + 1 >= questions.length) {
      // Quiz finished — calculate score and save to progress
  ...
  // ── Fallback quiz for custom/user-uploaded courses without a QUIZ_BANK entry ──
  ...
  // ── Call backend AI route to generate a summary using Gemini ──
  async function generateSummary() {
    setAiLoading(true);
    setAiSummary("");

    // Build prompt — use last watched video if available, otherwise use course title
    ...
    const prompt = lastVideo
      ...
      : `Summarize the key concepts of "${course?.title}" for a beginner student in 4-5 sentences. Focus on what skills or knowledge the student will gain.`;

    try {
      // POST to /api/ai/summary — auth token is attached automatically by api interceptor
      ...
      const msg = err.response?.data?.error
        ...
        || err.message
        || "Unknown error";
      setAiSummary(`Failed to generate summary: ${msg}`);
    } finally {
      setAiLoading(false);
    }
  }

  // ── Save the current note text for the active course ──
  function saveNote() {
    setNotes(n => ({ ...n, [activeCourse]: noteText }));
    alert("Note saved!");
  }

  // ── Overall statistics across all courses ──
  ...
  // ── Filter videos for the modal search (exclude already-added courses) ──
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ display: "flex" }}>
        ...
        <main style={{ flex: 1, padding: "24px", overflowX: "hidden", maxWidth: "1200px" }}>
          ...
          {/* ── Main content: course list (left) + active course panel (right) ── */}
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {courses.map(c => {
                  ...
                  const cpct   = Math.round((p.done / c.lessons) * 100);
                  ...
                  return (
                    <div key={c.id} className="course-card"
                      ...
                      {/* Clicking the card body opens that course */}
                      <div
                        onClick={() => { setActiveCourse(c.id); setTab("course"); setQuizState(null); setAiSummary(""); }}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                            <div style={{ flex: 1, height: "5px", background: "var(--border)", borderRadius: "999px", overflow: "hidden" }}>
                              <div style={{ height: "100%", width: `${cpct}%`, background: c.color, borderRadius: "999px", transition: "width 0.5s" }} />
                            </div>
                            <span style={{ fontSize: "11px", color: "var(--text-muted)", flexShrink: 0 }}>{cpct}%</span>
                          </div>
                          <p style={{ margin: "4px 0 0", fontSize: "11px", color: "var(--text-muted)" }}>
                            ...
                            {cpct === 100
                              ...
                              : cpct > 0
                              ? <span style={{ color: c.color, fontWeight: "600" }}>🔄 In Progress</span>
                              : "⬜ Not started"}
                          </p>
                        </div>
                      </div>
                      {/* Remove button — only shown for custom (user-added) courses */}
                      {c.custom && (
                        <button
            ...
            {activeCourse && course && (
              <div style={{ background: "var(--chip-bg)", border: "1px solid var(--border)", borderRadius: "16px", padding: "20px", display: "flex", flexDirection: "column", gap: "16px", animation: "fadeIn 0.2s ease" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <button onClick={() => setActiveCourse(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)" }}>
                    <FaTimes size={14} />
                  </button>
                </div>
                ...
                <div style={{ display: "flex", gap: "6px" }}>
                  {[
                    ...
                    <button key={t.id}
                      onClick={() => { setTab(t.id); setQuizState(null); }}
                      style={{
                      ...
                      {Array.from({ length: course.lessons }, (_, i) => {
                        ...
                        return (
                          <div key={i} className="lesson-row"
                            ...
                            {/* Complete button shown only on current lesson */}
                            {current && (
                ...
                {tab === "notes" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                    <p style={{ margin: 0, fontSize: "13px", color: "var(--text-muted)" }}>
                      Notes for <strong>{course.title}</strong>:
                    </p>
                    <textarea
                      value={noteText}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                    ...
                    <div style={{ padding: "14px", borderRadius: "10px", background: "linear-gradient(135deg, #e53e3e18, #7c3aed18)", border: "1px solid #e53e3e30" }}>
                      ...
                      <p style={{ margin: 0, fontSize: "12px", color: "var(--text-muted)" }}>
                        Powered by Gemini · summarizes your last watched video or course
                      </p>
                    </div>

                    {/* Generated summary text */}
                    {aiSummary && (
                      <div style={{ padding: "14px", borderRadius: "10px", background: "var(--bg)", border: "1px solid var(--border)", fontSize: "13px", lineHeight: "1.7", color: "var(--text)", whiteSpace: "pre-wrap" }}>
                        {aiSummary}
                      </div>
                    )}
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: aiLoading ? "var(--border)" : "linear-gradient(135deg, #e53e3e, #7c3aed)",
                    ...
                    <div key={video._id} className="video-pick-card"
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{ margin: 0, fontSize: "11px", color: "var(--text-muted)" }}>
                          {video.description?.slice(0, 60) || "No description"}{video.description?.length > 60 ? "…" : ""}
                        </p>
                      </div>
